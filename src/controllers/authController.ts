...
import { logAudit } from '../utils/audit';   // ← NEW IMPORT
  ...
  try {
    ...
    try {
      ...
      await logAudit({
        shopId,
        actorId: shopId,
        actorName: safeOwnerName || safeShopName,
...
export const login = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await logAudit({
      shopId: shop.id,
      actorId: shop.id,
      actorName: shop.owner_name || shop.shop_name,
      actorEmail: shop.email,
      action: 'LOGIN',           // Special action for login
      entityType: 'shop',
      entityId: shop.id,
      newValues: { login: true, email: shop.email },
      req
    });
