// import { Request, Response } from 'express';
// import { query, execute } from '../utils/db';
// import { genId } from '../utils/generateId';

// export const alertController = {
//   getAll: async (req: Request, res: Response) => {
//     const shopId = (req as any).shopId;
//     const data = await query('SELECT * FROM stock_alerts WHERE shop_id = ? ORDER BY created_at DESC', [shopId]);
//     res.json({ ok: true, data });
//   },

//   getUnread: async (req: Request, res: Response) => {
//     const shopId = (req as any).shopId;
//     const data = await query(
//       'SELECT * FROM stock_alerts WHERE shop_id = ? AND is_read = FALSE ORDER BY created_at DESC', 
//       [shopId]
//     );
//     res.json({ ok: true, data });
//   },

//   markRead: async (req: Request, res: Response) => {
//     const { id } = req.params;
//     const shopId = (req as any).shopId;
//     await execute('UPDATE stock_alerts SET is_read = TRUE WHERE id = ? AND shop_id = ?', [id, shopId]);
//     res.json({ ok: true });
//   },

//   markAllRead: async (req: Request, res: Response) => {
//     const shopId = (req as any).shopId;
//     await execute('UPDATE stock_alerts SET is_read = TRUE WHERE shop_id = ?', [shopId]);
//     res.json({ ok: true });
//   }
// };


import { Request, Response } from 'express';
import { genId } from '../utils/generateId';
import pool from '../config/db';

export const alertController = {
  getAll: async (req: Request, res: Response) => {
    const shopId = (req as any).shopId;

    try {
      const [data] = await pool.execute(
        'SELECT * FROM stock_alerts WHERE shop_id = ? ORDER BY created_at DESC',
        [shopId]
      );

      res.json({ ok: true, data });
    } catch (error: any) {
      console.error('Failed to fetch alerts:', error);
      res.status(500).json({ ok: false, error: 'Failed to fetch alerts' });
    }
  },

  getUnread: async (req: Request, res: Response) => {
    const shopId = (req as any).shopId;

    try {
      const [data] = await pool.execute(
        'SELECT * FROM stock_alerts WHERE shop_id = ? AND is_read = FALSE ORDER BY created_at DESC',
        [shopId]
      );

      res.json({ ok: true, data });
    } catch (error: any) {
      console.error('Failed to fetch unread alerts:', error);
      res.status(500).json({ ok: false, error: 'Failed to fetch unread alerts' });
    }
  },

  markRead: async (req: Request, res: Response) => {
    const { id } = req.params;
    const shopId = (req as any).shopId;

    const connection = await pool.getConnection();

    try {
      await connection.beginTransaction();

      const [result] = await connection.execute(
        'UPDATE stock_alerts SET is_read = TRUE WHERE id = ? AND shop_id = ?',
        [id, shopId]
      ) as any[];

      if (result.affectedRows === 0) {
        await connection.rollback();
        return res.status(404).json({ ok: false, error: 'Alert not found' });
      }

      await connection.commit();
      res.json({ ok: true, message: 'Alert marked as read' });
    } catch (error: any) {
      await connection.rollback();
      console.error('Failed to mark alert as read:', error);
      res.status(500).json({ ok: false, error: 'Failed to mark alert as read' });
    } finally {
      connection.release();
    }
  },

  markAllRead: async (req: Request, res: Response) => {
    const shopId = (req as any).shopId;

    const connection = await pool.getConnection();

    try {
      await connection.beginTransaction();

      await connection.execute(
        'UPDATE stock_alerts SET is_read = TRUE WHERE shop_id = ?',
        [shopId]
      );

      await connection.commit();
      res.json({ ok: true, message: 'All alerts marked as read' });
    } catch (error: any) {
      await connection.rollback();
      console.error('Failed to mark all alerts as read:', error);
      res.status(500).json({ ok: false, error: 'Failed to mark all alerts as read' });
    } finally {
      connection.release();
    }
  }
};